...
<style>
    /* Default zoom 80% */
    .stApp { background-color: #FFFFFF; zoom: 0.8; }
    ...
    .metric-card h2 { color: #1A365D; margin: 0; font-size: 1.8rem; }
    .metric-card p  { color: #718096; margin: 4px 0 0; font-size: 0.9rem; }
...
@st.cache_data
def load_data():
    # Path data: letakkan Data.xlsx di folder yang sama dengan script ini
...
with st.sidebar:
    ...
    st.markdown("""
    <div>
        <span class="legend-pill" style="background:#FFE5E5;color:#C53030;">● Merah &gt; 0.6</span><br><br>
        <span class="legend-pill" style="background:#FEFCBF;color:#744210;">● Kuning &gt; 0.55–0.6</span><br><br>
        <span class="legend-pill" style="background:#C6F6D5;color:#22543D;">● Hijau &gt; 0.5–0.55</span>
    </div>
    """, unsafe_allow_html=True)
...
colors = ["#3182CE" if p == selected_prov else "#BEE3F8" for p in eci_df["Provinsi"]]
...
fig_eci.update_layout(
    ...
    xaxis=dict(title="ECI Value", showgrid=True, gridcolor="#EDF2F7", zeroline=False),
    yaxis=dict(title=""),
    plot_bgcolor="#FFFFFF",
...
with col_net:
    ...
    net = Network(height="480px", width="100%", bgcolor="#FFFFFF", font_color="#2D3748")
    ...
    for _, row in df_prov.iterrows():
        ...
        for p in [p1, p2]:
            if p not in nodes_added:
                net.add_node(
                    p, label=p,
                    color="#3182CE", size=18,
                    font={"size": 13, "color": "#1A365D", "face": "Inter"},
                    borderWidth=2, borderWidthSelected=4,
                    shadow=True,
                )
                nodes_added.add(p)
        edge_col = EDGE_COLOR.get(str(row["Rule Strength"]), "#A0AEC0")
        net.add_edge(
            p1, p2,
            color={"color": edge_col, "highlight": edge_col, "opacity": 0.85},
            width=3,
    ...
    tmp_path = tmp.name
    tmp.close()  # Close before PyVis writes to it (required on Windows)
    ...
    try:
        ...
        pass  # Safe to ignore if cleanup fails on Windows

    # Inject white background override
